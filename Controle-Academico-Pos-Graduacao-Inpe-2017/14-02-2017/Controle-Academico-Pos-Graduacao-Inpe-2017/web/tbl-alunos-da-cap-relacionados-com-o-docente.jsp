...
<head>
    ...
    <script type="text/javascript">
        function criarArquivoCSV(){
            var data = "Código;Nome;Nível;Status;Sigla Curso;Data Admissão;O_PESQ1;O_PESQ2;O_PESQ3;O_PESQ4\n";
            /*****************************************************************************************
             ******************************************************************************************/
            <%
                Database academxp_2 = DatabaseBuilder.open(new File(request.getServletContext().getRealPath("/")+"Data/Academxp.mdb"));
                TreeMap<String, TreeMap<String, String>> mapa_resultado = Main.obterAlunosAtivosDoCAPAPartirDoCodigoDocente(request.getParameter("codigoDocente"), academxp_2.getTable("CURSO_AL"), academxp_2.getTable("GDRPESS"));
            ...
            %>
            data = data+"<%=codigo_aluno%>"+";"+"<%=dados_aluno.get("NOME") %>"+";"+"<%=dados_aluno.get("NIVEL")%>"+";"+"<%=dados_aluno.get("STATUS")%>"+";"+"<%=dados_aluno.get("SIGLA_CURS")%>"+";"+"<%=dados_aluno.get("DATA_ADMISSAO")%>"+";"+"<%=dados_aluno.get("O_PESQ1")%>"+";"+"<%=dados_aluno.get("O_PESQ2")%>"+";"+"<%=dados_aluno.get("O_PESQ3")%>"+";"+"<%=dados_aluno.get("O_PESQ4")%>"+"\n";
            <% } %>
            /*****************************************************************************************
             ******************************************************************************************/
    ...
    <script>
        $(document).ready(function(){
            $("#tbl-alunos-da-cap-relacionados-com-o-docente").DataTable();
        });
    </script>
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div style="margin-bottom: 10px;">
                    <a  href="grafico-estimativa-tempo-alunos.jsp?codigoDocente=<%=request.getParameter("codigoDocente")%>" class="btn btn-default " onclick="">
                        <span class="glyphicon glyphicon-stats"></span> Estimativa
                    </a>
                ...
                <table class="table table-striped table-bordered table-hover" id="tbl-alunos-da-cap-relacionados-com-o-docente">
                    <thead>
                        <tr>
                            <th>Código</th>
                            <th>Nome</th>
                    ...
                    <tbody>
                         <% 
                            ...
                            TreeMap<String, TreeMap<String, String>> mapaResultado = Main.obterAlunosAtivosDoCAPAPartirDoCodigoDocente(request.getParameter("codigoDocente"), academxp.getTable("CURSO_AL"), academxp.getTable("GDRPESS"));
